...
#define USB_MSC_PROTOCOL_BBB        0x50    // Bulk-Only Transport ("BBB")

// Class-specific control requests (bmRequestType = 0xA1 / 0x21)
#define USB_MSC_REQ_GET_MAX_LUN     0xFE
#define USB_MSC_REQ_RESET           0xFF

//
// Bulk-Only Transport wire formats (USB MSC BOT 1.0)
//
...
// Command Block Wrapper — 31 bytes, sent on bulk-OUT.
struct usb_msc_cbw {
    uint32_t signature;
    uint32_t tag;
    ...
    uint8_t  cdb_length;        // bits 4:0; valid range 1..16
    uint8_t  cdb[16];           // SCSI CDB, zero-padded
} __attribute__((packed));

// Command Status Wrapper — 13 bytes, received on bulk-IN.
struct usb_msc_csw {
    uint32_t signature;
    uint32_t tag;
    uint32_t data_residue;      // bytes the device did not transfer
...
//
// SCSI op codes we use (transparent command set)
//
...
struct scsi_inquiry_data {
    ...
    uint8_t  reserved[3];
    char     vendor[8];             // ASCII, space-padded, NOT null-terminated
    char     product[16];
    char     revision[4];
} __attribute__((packed));
...
struct usb_msc_dev {
    ...
    uint8_t            iface;           // interface number (always 0 in our parser)
    ...
    uint32_t           block_size;      // bytes per LBA (typically 512)
...
// Dump all bound MSC devices (one line each).
